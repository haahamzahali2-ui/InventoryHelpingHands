...
function openVisitNoteModal() {
  if (!currentPatientId) return;
  document.getElementById('visitNoteModalSub').textContent = `Patient #${currentPatientId} · Powered by Gemini AI`;
...
async function generateVisitNote() {
  ...
  const discMedList = allMeds.filter(m => m.endDate && new Date(m.endDate) < new Date())
    .map(m => `  ${m.name} ${m.dosage} (discontinued ${m.endDate})`).join('\n');
...
Write a thorough clinical paragraph that: (1) states the reason for visit and how long patient has been followed, (2) describes current vital findings with exact numbers and comparisons to prior readings, (3) assesses the trend and medication effectiveness, (4) identifies any urgent concerns, (5) outlines specific follow-up recommendations.`;
  ...
  try {
    const params = new URLSearchParams({
      ...
      payload: JSON.stringify({ prompt, isSOAP, patientId: currentPatientId })
    });
    const res = await fetch(`${sheetWriteUrl}?${params}`, { method: 'GET', redirect: 'follow' });
    const raw = await res.text();
    console.log('GAS generate_note raw response:', raw);
    const data = JSON.parse(raw);
    console.log('GAS generate_note parsed:', data);
    if (!data.ok) throw new Error(data.error || 'Unknown error from GAS');
    const text = data.note || 'Unable to generate note. Please try again.';
