...
function getMedAnnotations(medications, readings) {
  ...
  const annotations = [];
  medications.filter(m => m.startDate).forEach(m => {
    const startIdx = labels.findIndex(d => d >= m.startDate);
    if (startIdx >= 0) {
      annotations.push({ medName: m.name, label: `💊 ${m.name} started`, idx: startIdx, date: m.startDate, type: 'start' });
    }
    if (m.endDate) {
      let endIdx = labels.findIndex(d => d >= m.endDate);
      if (endIdx < 0) endIdx = labels.length - 1;
      annotations.push({ medName: m.name, label: `🚫 ${m.name} ended`, idx: endIdx, date: m.endDate, type: 'end' });
    }
  });
  return annotations;
}
...
window.renderBPChart = function (readings) {
  ...
  annotations.forEach(ann => {
    bpChartInst.data.datasets.push({
      ...
      borderColor: ann.type === 'end' ? 'rgba(200,80,80,0.6)' : 'rgba(168,137,60,0.6)',
      borderWidth: 2,
      borderDash:  [6, 3],
      pointRadius: 0,
      type:        'line',
  ...
  const plugin = {
    ...
    afterDraw(chart) {
      ...
      annotations.forEach(ann => {
        ...
        ctx.stroke();
        ctx.setLineDash([]);
        const vertOffset = annotations.indexOf(ann) % 3;
